...
class Dispatch {
    ...
    private function generic_domain_parse($request_uri) {
        ...
        if ($generic_domain == false) return $request_uri; //没有设置泛解析，直接返回$request_uri
        //获取泛解析排除设置
        ...
        if ($request_uri == '/') $request_uri = '/Index'.$this->url_separator.'index';
        $request_uri = trim($request_uri, '/');
    ...
    private function group_ban() {
        $generic_domain = C('generic_domain'); //获取泛解析设置
        if ($generic_domain == false) return true; //没有设置泛解析，返回true
        //获取泛解析排除域名
        if (!empty($generic_domain['exclude'])) {
            foreach ($generic_domain['exclude'] as $v) {
    ...
    private function special_url($path_info) {
        ...
        if ($special_url == false) return $path_info;
