...
class GF_View {
    ...
    public function processTemplate($tpl = null) {
        ...
        if (!file_exists($template_path))
            sysError('要载入的模板文件不存在！' . $template_path);
        else
        ...
        if (file_exists($cache_template_path) == true) {
            ...
            //模板更新，重新创建模板缓存
            if ($template_mtime > $cache_template_mtime)
                $parseTemplateStatus = 1;
        }
        else {
        ...
        if ($parseTemplateStatus == 0) {
            if (C('layout') == true) {
                ...
                if ($layout_mtime > $cache_template_mtime)
                    $parseTemplateStatus = 1;
            }
        }
        ...
        if ($parseTemplateStatus == 0) {
            ...
            if (!empty($include_url_array)) {
                foreach ($include_url_array as $k => $v) {
                    if (!file_exists($v))
                        sysError('要载入的模板文件不存在！' . $v);
                    else {
                        $include_template_mtime = filemtime($v);
                        //如果模板更新，重新创建模板缓存
                        if ($include_template_mtime > $cache_template_mtime)
                            $parseTemplateStatus = 1;
                    }
                }
            }
        ...
        if ($parseTemplateStatus == 1 || C('debug') == true)
            $this->cacheTemplate($template_path);
    ...
    public function cacheTemplate($template_path) {
        ...
        if ($this->saveTemplateCache($cache_template_path, $content))
            return $cache_template_path;  //生成php缓存文件，返回cache路径
        else
            sysError('Template cache file does not save !  url : ' . $cache_template_path);
    }
    ...
    private function getTemplateContent($template_path) {
        if (!file_exists($template_path))
            sysError('Template does not exist !');
        else {
            ...
            if ($pos === 0 or $pos > 0) {
                ...
                if (!empty($this->layout))
                    $content = str_replace('{__CONTENT__}', $content, $this->layout);
                return $content;
            }
        }
    }
    ...
    private function _parseInclude($content) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (file_exists($filePath))
                ...
                return '要包含的文件不存在<br>URL：' . $filePath;
